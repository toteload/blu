...
SEGMENTLIST_LINKAGE SEGMENTLIST_TYPE  Cat(SEGMENTLIST_FUNCTION_PREFIX, _pop)(SEGMENTLIST_NAME *list);
...
SEGMENTLIST_LINKAGE
SEGMENTLIST_TYPE Cat(SEGMENTLIST_FUNCTION_PREFIX, _pop)(SEGMENTLIST_NAME *list) {
  Assert(list->len > 0);
  SEGMENTLIST_TYPE res = Cat(SEGMENTLIST_FUNCTION_PREFIX, _at_unchecked)(list, list->len-1);
  list->len -= 1;
  return res;
}
