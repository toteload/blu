...
internal void frame_pop(Interpreter *in) {
  ...
  for (u32 i = 1; i < f->chunk->opcode_count; i++) {
    if (f->inst_values[i]) {
      values_dealloc(&in->compiler->values, f->inst_values[i]);
    }
  }
...
internal u32 step(Interpreter *in) {
  ...
  case IR_condbr: {
    ...
    Value *v = values_get(&in->compiler->values, resolve(in, f, condbr->cond));
    if (*Cast(u8*,v->data)) {
      f->pc = condbr->then;
    } else {
  ...
  case IR_call: {
    ...
    Value *v = values_get(&in->compiler->values, resolve(in, f, call->func));
